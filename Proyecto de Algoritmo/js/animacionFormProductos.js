...
//---------------- BUCAR POR CATEGORIAS LOS PRODUCTOS-PAGINA-2 ------------------//


document.querySelector("#buscador").addEventListener("change", e => {
    const valorSeleccionado = e.target.value.toLowerCase();
  
    document.querySelectorAll(".category").forEach(fruta => {
       
        if( fruta.textContent.toLowerCase().includes(valorSeleccionado)){
